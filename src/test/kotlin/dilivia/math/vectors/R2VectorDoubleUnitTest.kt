...
@DisplayName("R2VectorDouble Tests")
internal class R2VectorDoubleUnitTest {

    @DisplayName("Operations.")
    @ParameterizedTest(name = "{index}: {argumentsWithNames}")
    @EnumSource(R2VectorDoubleOperationsTest::class)
    fun operations(test: R2VectorDoubleOperationsTest) {
    ...
    enum class R2VectorDoubleOperationsTest(
            val v1: R2VectorDouble,
            val v2: R2VectorDouble,
            val equals: Boolean,
            val compareTo: Int,
            ...
            val crossProduct: Double,
            val dotProduct: Double,
            val angle: Double
    ) {

        TEST1(
                v1 = R2VectorDouble(3.0, 2.0),
                v2 = R2VectorDouble(2.0, 3.0),
                equals = false,
                compareTo = 1,
                approxEquals = false,
                sum = R2VectorDouble(5.0, 5.0),
                subtraction = R2VectorDouble(1.0, -1.0),
                multiplication = R2VectorDouble(6.0, 6.0),
                division = R2VectorDouble(3.0 / 2.0, 2.0 / 3.0),
                crossProduct = 3.0 * 3.0 - 2.0 * 2.0,
                dotProduct = 3.0 * 2.0 + 2.0 * 3.0,
                angle = 0.3947911196997615
        ),

        TEST2(
                v1 = R2VectorDouble(3.0, 0.0),
                v2 = R2VectorDouble(2.0, 3.0),
                equals = false,
                compareTo = 1,
                approxEquals = false,
                sum = R2VectorDouble(5.0, 3.0),
                subtraction = R2VectorDouble(1.0, -3.0),
                multiplication = R2VectorDouble(6.0, 0.0),
                division = R2VectorDouble(3.0 / 2.0, 0.0),
                crossProduct = 3.0 * 3.0 - 2.0 * 0.0,
                dotProduct = 3.0 * 2.0,
                angle = 0.982793723247329
        ),

        TEST3(
                v1 = R2VectorDouble(1.0, 2.0),
                v2 = R2VectorDouble(2.0, 0.0),
                equals = false,
                compareTo = -1,
                approxEquals = false,
                sum = R2VectorDouble(3.0, 2.0),
                subtraction = R2VectorDouble(-1.0, 2.0),
                multiplication = R2VectorDouble(2.0, 0.0),
                division = R2VectorDouble(1.0 / 2.0, POSITIVE_INFINITY),
                crossProduct = 1.0 * 0.0 - 2.0 * 2.0,
                dotProduct = 1.0 * 2.0,
        ...
        TEST4(
                v1 = R2VectorDouble(3.0, 2.0),
                v2 = R2VectorDouble(3.0, 2.0),
                equals = true,
                compareTo = 0,
                approxEquals = true,
                sum = R2VectorDouble(6.0, 4.0),
                subtraction = R2VectorDouble(0.0, 0.0),
                multiplication = R2VectorDouble(9.0, 4.0),
                division = R2VectorDouble(1.0, 1.0),
                crossProduct = 0.0,
                dotProduct = 9.0 + 4.0,
                angle = 0.0
        ),
        ...
        TEST5(
                v1 = R2VectorDouble(3.0, 2.0),
                v2 = R2VectorDouble(3.0 + 1e-14, 2.0 + 1e-14),
                equals = false,
                compareTo = -1,
                approxEquals = true,
                sum = R2VectorDouble(6.000000000000011, 4.000000000000011),
                subtraction = R2VectorDouble(-1.021405182655144E-14, -1.021405182655144E-14),
                multiplication = R2VectorDouble(3.0 * (3.0 + 1e-14), 2.0 * (2.0 + 1e-14)),
                division = R2VectorDouble(3.0 / (3.0 + 1e-14), 2.0 / (2.0 + 1e-14)),
                crossProduct = 3.0 * (2.0 + 1e-14) - (3.0 + 1e-14) * 2.0,
                dotProduct = 3.0 * (3.0 + 1e-14) + 2.0 * (2.0 + 1e-14),
                angle = 7.515355859001031E-16
        ),
    }
